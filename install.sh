...
#   1. copies verify.sh + lib.sh + impact.sh + claim.sh + hypothesis.sh + memory.sh + experiment.sh + mode.sh + guards.d/ + templates/ into .proofgate/
...
vendor
chmod +x "$DEST/verify.sh" "$DEST/impact.sh" "$DEST/claim.sh" "$DEST/hypothesis.sh" "$DEST/memory.sh" "$DEST/experiment.sh" "$DEST/mode.sh" "$DEST"/guards.d/*.sh 2>/dev/null || true
