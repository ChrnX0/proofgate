...
| "I ran `cmd \| tail` (or in the background), exit 0, the end of the output was clean" | **You read someone else's exit code.** A pipeline reports the LAST command's (`tail`), and `$?` right after `cmd &` reports that the FORK started, not how the job ended — while the tail of the output is the package manager's update notice, forty lines below the errors. Write the output to a FILE and read the status of the command itself: `cmd > out.txt 2>&1; echo $?` in the foreground; `cmd > out.txt 2>&1 & pid=$!; wait "$pid"; echo $?` in the background (a backgrounded pipeline needs `set -o pipefail` too, or it hands you `tail`'s status again). Then COUNT the signal (`grep -c "error TS"`, `grep -c FAIL`), and **sabotage on purpose** (one import that does not exist) to watch the check catch it: a check that has never flagged anything has not proven it can see. A typecheck with 300+ errors was declared green this way; the test suite caught it |
| "The mutation run says the tests catch everything" | **What judged it: the exit code, or a pattern in the output?** Five mutations once reported SURVIVED — one of them re-introducing the exact regression the tests had just been written for — because the judge was `runner \| tail -3 \| grep -q failed`, and `tail -3` cuts above the `Tests N failed` line. Two sanity locks, always together: (a) run the **UNMUTATED baseline** and require green — if it isn't, everything after is noise; (b) include a **harmless sentinel** (change a comment) that MUST survive. A dead sentinel means the judge fails everything; if EVERYTHING survives alongside it, the judge passes everything |
| "I faked the clock in the test and the time window matched" | **Which clock does each timestamp come from?** A fake-timer moves the process clock only: a column default (`datetime('now')`, `now()`) is the DATABASE's clock, and an application `new Date()` is a third. Ten rows once landed in the "answered within 24h" bucket because arrival came from the database (real time) and contact from the faked one — which also hid a PRODUCTION defect, where the resulting negative interval was clamped to "zero hours". Pin both ends at their real source, and decide what the IMPOSSIBLE case is worth before it reaches production: not knowing is an answer, guessing is not |
| "I re-sign the payload before resending it" | **Can you reproduce the exact text you signed?** A signature covers BYTES. A `jsonb`/`json` column has license to rewrite what you stored — Postgres jsonb reorders keys and strips whitespace — so rebuilding the body from it yields a different string, a different digest, and a subscriber that rejects every redelivery. The trap is that a development database storing that column as TEXT round-trips parse+stringify byte-identical **by accident**: no local test can see the divergence, and the defect debuts in production. Keep the signed text in a `text` column and send THAT; a row without it is not retryable and should say so, rather than going out with a signature that cannot match |
...
| "The webhook verifies its signature, so the endpoint is safe" | **Against WHICH bytes?** An HMAC covers the body that arrived. Parsing first and hashing `JSON.stringify(parsed)` digests a different text — one space, one key order, one unicode escape apart — so every legitimate delivery is rejected, and the "fix" someone reaches for is deleting the check. Read the body ONCE as text, hash that text, parse the JSON out of it. Then drive the negative path for real: no signature, and a signature made with the wrong secret, each asserting that NOTHING was written. Also check the comparison itself — `timingSafeEqual` THROWS on different lengths, so a forged short signature becomes a 500 with a stack trace instead of a 401 |
