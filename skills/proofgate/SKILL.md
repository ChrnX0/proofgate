...
| "I ran `cmd \| tail` (or in the background), exit 0, the end of the output was clean" | **A pipeline's exit code is the LAST command's** (`tail`), and the tail of the output is the package manager's update notice — the errors sit forty lines above it. Write the output to a FILE, check the command's own `$?`, and COUNT the signal (`grep -c "error TS"`, `grep -c FAIL`). Then **sabotage on purpose** (one import that does not exist) and watch the check catch it: a check that has never flagged anything has not proven it can see. A typecheck with 300+ errors was declared green this way; the test suite caught it |
